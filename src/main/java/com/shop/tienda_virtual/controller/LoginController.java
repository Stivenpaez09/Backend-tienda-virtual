...
public class LoginController {
    ...
    @PostMapping
    public ResponseEntity<Map<String, String>> createLogin(@RequestBody RegisterRequestDTO login) {
        loginService.createLogin(login);
        return ResponseEntity.status(HttpStatus.CREATED).body(Map.of("message", "Login creado con éxito"));
    }

    @GetMapping
    public ResponseEntity<List<LoginProfileDTO>> getLogins() {
    ...
    @GetMapping("/{id_login}")
    public ResponseEntity<LoginProfileDTO> findLogin(@PathVariable("id_login") Long id_login) {
        return ResponseEntity.ok(loginService.findShowLogin(id_login));
    }
